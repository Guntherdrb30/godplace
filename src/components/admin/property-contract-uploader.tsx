...
export function AdminPropertyContractUploader(props: {
  propertyId: string;
  url: string | null;
  ...
  const subir = async (file: File) => {
    ...
    try {
      ...
      const cr = await fetch("/api/admin/property_contract/upsert", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ propertyId: props.propertyId, url: upData.url, pathname: upData.pathname }),
      });
  ...
  return (
    <div className="space-y-3">
      <div className="grid gap-2">
        <Label htmlFor="admin-prop-contract">Subir contrato</Label>
        <Input
          id="admin-prop-contract"
          type="file"
