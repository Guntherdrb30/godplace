    ...
    try {
      ...
      if (!res.ok) {
        toast("No se pudo crear la reserva.", {
          description: data?.message || "Intenta nuevamente o inicia sesion.",
        });
        return;
      }

      toast("Reserva creada como borrador.", {
        description: "MVP: pagos reales pendientes. Aun requiere confirmacion operativa.",
      });
    } finally {
      setLoading(false);
    }
    ...
    <div className="rounded-3xl border bg-white/85 p-6 shadow-suave">
      ...
      <div className="mt-5 grid gap-4">
        ...
        <div className="grid gap-2">
          ...
          />
          <div className="text-xs text-muted-foreground">Maximo: {props.maxGuests}</div>
        </div>

        <div className="rounded-2xl border bg-secondary/40 p-4 text-sm">
          {!quote?.ok ? (
            <div className="text-muted-foreground">
              {loading ? "Cotizando..." : quote?.message || "Selecciona fechas para ver el total."}
            </div>
          ) : (
        ...
        <Button type="button" variant="brand" onClick={reservar} disabled={loading || !quote?.ok}>
          Reservar
        </Button>
